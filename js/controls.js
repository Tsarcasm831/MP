...
export class PlayerControls {
  ...
  setupEventListeners() {
    ...
    document.addEventListener("keydown", (e) => {
      // Prevent movement keys when an input field is active
  ...
  processMovement() {
    ...
    function checkCollision(block, overrideWidth, overrideHeight, overrideDepth) {
      ...
      // Check for standing on top of a block
      if (
        this.velocity.y <= 0 &&
        y >= blockTop - 0.2 && // Player is at or just above the block
        newY <= (blockTop + 0.20) && // Player is moving down onto the block, account for the raise.
        Math.abs(newX - block.position.x) < (blockWidth / 2 + playerRadius) &&
        Math.abs(newZ - block.position.z) < (blockDepth / 2 + playerRadius)
      ) {
        standingOnBlock = true;
        newY = blockTop + 0.20; // Set player slightly above the object top
        this.velocity.y = 0;
        this.canJump = true;
      } else if ( // Check for side collisions
        Math.abs(newX - block.position.x) < (blockWidth / 2 + playerRadius) &&
        Math.abs(newZ - block.position.z) < (blockDepth / 2 + playerRadius) &&
